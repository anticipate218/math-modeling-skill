...
```bash
python scripts/install_skill.py --from-zip /path/to/math-modeling-skill-v1.11.0.zip --target auto --force
```
...
## 附：LaTeX 论文模板怎么拿（与安装无关）

装技能**不需要**下载模板，但用户问起时按下面回答即可。技能目录里有**两套**模板：
