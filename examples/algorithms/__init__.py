...
- ``optimization``    线性/整数规划（单纯形）、指派、运输、背包、目标规划、
                      鲁棒 LP、机会约束 LP、离散选址
- ``evaluation``      层次分析、熵权、CRITIC、TOPSIS、VIKOR、灰关联、DEA、模糊综合
- ``forecasting``     移动平均、指数平滑、Holt 线性/季节、AR 最小二乘、
                      ACF/PACF/ADF、误差指标、滚动原点回测、季节分解
- ``timeseries``      GM(1,1) 与后验差检验、ARIMA/SARIMA、GARCH(1,1)、
                      卡尔曼滤波（局部水平与一般线性）、Ljung-Box 白噪声检验
- ``statistics``      最小二乘、岭回归、lasso、logistic、Poisson、PCA、因子分析、
                      Bootstrap、置换检验、卡方、逐步回归、异方差/自相关诊断
- ``ml``              数据划分与分层抽样、标准化、混淆矩阵/宏平均指标/ROC-AUC、
                      kNN、CART、随机森林、梯度提升、高斯朴素贝叶斯、LDA、
                      置换重要性、SMOTE、类别权重
- ``clustering``      K-means++、层次聚类、DBSCAN、GMM-EM、谱聚类、模糊 C 均值、
                      轮廓系数、DB/CH 指数、肘部曲线、Gap 统计量
- ``multicriteria``   PROMETHEE II、ELECTRE I/III、秩和比（RSR）与概率单位分档、
                      Borda、Copeland、排序一致性
- ``multiobjective``  支配关系、快速非支配排序、拥挤距离、加权和法、ε 约束法、
                      NSGA-II、二维超体积、理想点距离
- ``sensitivity``     单因素（OAT）敏感性与弹性、Morris 筛选、Sobol 一阶/总效应、
                      缺失值插补（均值/kNN/迭代回归）、离群点检测（Z/IQR/MAD）
- ``differential``    Euler/RK4/RK45、SIR/SEIR、R0、隐式欧拉、logistic 增长与混沌、
                      Lotka-Volterra、参数拟合、平衡点稳定性
- ``stochastic``      蒙特卡洛积分与置信区间、M/M/c 与 M/G/1 排队、离散事件仿真、
                      马尔可夫稳态、随机游走、MCMC、Copula
- ``graphs``          Dijkstra、Floyd、Prim/Kruskal、最大流、最小费用流、
                      二分图匹配、A*、VRP、PageRank、中心性、社区发现、网络鲁棒性
- ``game``            零和博弈 LP、双矩阵纳什、Shapley 值、Gale-Shapley 稳定匹配、
                      复制子动态、α-β 剪枝、Stackelberg 主从博弈、纳什谈判解
...
- ``spatial``         一维热传导（显式/隐式）、二维泊松 SOR、森林火灾元胞自动机、
                      NaSch 交通流、Buckingham π 定理、相似缩放
"""
